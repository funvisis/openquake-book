...
deterministic earthquake, for a collection of assets (see Figure \ref{Scheme_detrisk_calc}). Such analyses are of importance, for example, for emergency management planning and for raising societal awareness of risk. 
...
assets, based on the probabilistic hazard (see Figure \ref{Scheme_probrisk_calc}). The losses are calculated with an event-based approach, such that the simultaneous losses to a set of assets can be calculated.
...
single assets, based on the probabilistic hazard (see Figure \ref{Scheme_classrisk_calc}). The output of this calculator is useful for comparative risk assessment between assets at different locations.
\end{itemize}
